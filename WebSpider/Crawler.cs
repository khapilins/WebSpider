...
namespace WebSpider
{
    ...
    {
        public static String _connectionstring = "Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=\"C:\\Users\\khapi\\Documents\\Visual Studio 2015\\Projects\\WebSpider\\WebSpider\\Indexes.mdf\";Integrated Security=True;Connect Timeout=120";
        ...
        private void Log(Exception ex)
        {
            ...
            to_write.Append("\n"); to_write.Append(URL);to_write.Append("\n");
            to_write.Append(ex.Message);to_write.Append("\n");
            sr.Write(to_write);
            sr.Close();
        }
        private void FlushURL(Guid current_url_Guid,DBQueryQuee db_quee)
        {
            ...
            //{
                ...
                batch.Append(URL);batch.Append("') INSERT INTO URLLIST (Id, url) values ('");
                batch.Append(current_url_Guid);batch.Append("', '");batch.Append(URL);batch.Append("') ");
                //  batch.Append(Environment.NewLine);            
            ...
            {                
                    ...
                    batch.Append("if not exists(select * from UrlList where url='");
                    batch.Append(LINKS[i]);batch.Append("') INSERT INTO URLLIST (Id, url) values ('");
                    batch.Append(next_url_ids[i]);batch.Append("', '");batch.Append(LINKS[i]);batch.Append("')");
                    db_quee.Queries.Enqueue(batch.ToString());
                    //batch.Append(Environment.NewLine);
                    batch = new StringBuilder();
        ...
        private void FlushWords(Guid current_url_Guid,DBQueryQuee db_quee )
        {
            ...
            s_builder.Append(ExtractedParagraphs); s_builder.Append(ExtractedDivs); s_builder.Append(ExtractedLinksText);
            ...
            for (int i = 0; i < word_ids.Length; ++i)
            {
                ...
                batch.Append("if not exists(select * from WordList where word='");
                batch.Append(words[i].ToLower());batch.Append("') INSERT INTO WordList (Id, Word) values ('");
                batch.Append(word_ids[i] + "', '" + words[i].ToLower());batch.Append("')");
                db_quee.Queries.Enqueue(batch.ToString());
                batch = new StringBuilder();
                //batch.Append(Environment.NewLine);
            }
            ...
            for (int i = 0; i < word_ids.Length; ++i)
            {
                ...
                batch.Append(current_url_Guid);batch.Append("' and ");
                batch.Append("WordID='");batch.Append(word_ids[i] + "' and WordLocation='" + (i + 1).ToString());
