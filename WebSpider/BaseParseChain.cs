...
namespace WebSpider
{
    public class BaseParseChain
    {
        ...
        public void Add(BaseParseChain next) { if (Next != null) { Next.Add(next); } else { Next = next; } }
        virtual public void SendNext(Crawler c) { if(this.Next!=null)this.Next.SendNext(c); }
    ...
    public class ParseLinksChain : BaseParseChain
    {
        ...
        private void ExtractLinks(Crawler c)
        {
            ...
            foreach (Match m in res)
            {
                link = ExtractSingleLink(m.ToString(),c.URL);
        ...
        private string ExtractSingleLink(string href_link, string start_url)
        {
            ...
            if (!String.IsNullOrEmpty(href_link))
            {
                ...
                {
                    result_link.Append("http:"); result_link.Append(href_link);
                }
